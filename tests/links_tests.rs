...
fn test_build_anchor_map_returns_line_offsets() {
    ...
    assert!(
        map.contains_key("introduction"),
        "should have 'introduction' key"
    );
    assert!(map.contains_key("details"), "should have 'details' key");
    assert!(
        map["introduction"] < map["details"],
        "introduction should come before details"
    );
}
...
fn test_full_readme_collect_links_count() {
    ...
    // Should find at least 9 links from TEST.md Further reading
    assert!(
        links.len() >= 9,
        "expected ≥9 links, got {}: {links:?}",
        links.len()
    );
}
